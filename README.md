...
This is a service providing an MDTP proxy to the **nonrep-retrieval** service.
...
In order to run the microservice locally, you must have SBT installed. You should then be able to start the application using:

> ```sbt run {PORT}```

(_The default port used for this project is:_ 9391)

To run the tests for the application, you can run: 
> ```sbt clean test```

To run the integration tests for the application, you can run: 
> ```sbt it/test```
...
Landing page URL for the service is 
> ```https://{HOST:PORT}/nrs-retrieval/start```
...
- If the request is authenticated by `stride` but does not have the enrolment `nrs_digital_investigator` then `403 FORBIDDEN` is returned

- If the request is not authenticated by `stride` then `401 UNAUTHORIZED` is returned
